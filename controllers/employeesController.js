...
const updateEmployee = async (req, res) => {
	...
	if (!employee) return res.status(204).json({ message: `No employee ID matches id ${req.body.id}` })
...
const getEmployee = async (req, res) => {
	...
	const employee = await Employee.findOne({ _id: req.body.id }).exec()
	if (!employee) return res.status(204).json({ message: `No employee ID matches id ${req.params.id}` })
...
/*  COMO QUE ERA ANTES COM O DATABASE LOCAL INVES DO MONGO
